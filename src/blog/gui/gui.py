...
class RemovePanel:
	...
	def create_widget(self):
		...
		self.removebutton = Button(self.canvas,text="开始备份",height = 2,
		font = self.second_font,command=self.remove_drm)
...
class SimpleFileChoose:
	...
	def __init__(self,master,x,y,width,height):
		...
		#temp_dir.encode("gbk")
		try:
			self.outputdir = toplevel.EncodeUTF8(toplevel.GetNativeDecode(toplevel.realdir+"\\output"))
		except Exception,e:
		...
		options['filetypes'] = [('PDF files', '.pdf'),('ePub files', '.epub'),('all files', '.*')]
		#options['initialdir'] = 'E:\\'
		#options['initialfile'] = 'myfile.txt'
		options['parent'] = self.canvas
		options['title'] = 'Select PDF or ePub File'
